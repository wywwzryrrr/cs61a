...
class VendingMachine:
    ...
    def restock(self, number):
        ...
        return 'Current {0} stock: {1}'.format(self.product, self.stock_number)
        
    def add_funds(self, money):
        if self.stock_number == 0:
            return 'Inventory empty. Restocking required. Here is your ${0}.'.format(money)
        self.money += money
        return 'Current money : ${0}'.format(self.money)
...
def store_digits(n):
    """Stores the digits of a positive number money in a linked list.
...
class Tree:
    ...
    def __str__(self):
        def print_tree(t, indent=0):
            tree_str = '  ' * indent + str(t.label) + "\money"
            for b in t.branches:
