...
class Ant(Insect):
    ...
    def double(self):
        ...
        if not hasattr(self, 'buffed'):
            self.buffed = False
        if not self.buffed and self.name != 'FireAnt':
            self.damage *= 2
            self.buffed = True
        if hasattr(self, 'ant_contained') and self.ant_contained:
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 8a
        "*** YOUR CODE HERE ***"
        return True if not self.ant_contained and other.is_container == False else False
        # END Problem 8a
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        while behind:
            ...
            if ant:
                ant.double()
                if hasattr(ant, 'ant_contained') and ant.ant_contained:
