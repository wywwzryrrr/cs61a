...
def swine_align(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 4a
    "*** YOUR CODE HERE ***"
    big = max(player_score, opponent_score)
    small = min(player_score, opponent_score)
    gcd = 0
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        if who == 0:
            ...
            if highest > running_high and highest > 0:
                print(highest,'point(s)! The most yet for Player 0')
                return announce_highest(who, score0, highest)
            else:
                return announce_highest(who, score0, running_high)
        if who == 1:
            highest = score1 - last_score
            if highest > running_high and highest > 0:
                print(highest,'point(s)! The most yet for Player 1')
                return announce_highest(who, score1, highest)
            else:
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    i, max_i = 1, 0
    score, max_score = 0, 0
    max_average = make_averaged(roll_dice)
    while i <= 10:
        score = max_average(i, dice)
        if score > max_score:
            max_score = score
            max_i = i
        i += 1
